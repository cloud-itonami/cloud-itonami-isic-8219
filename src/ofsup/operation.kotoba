(ns ofsup.operation
  "OperationActor -- one office-support-service coordination operation
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (OfficeSupport-LLM) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Office Support
  ...
  One graph run = one coordination operation (intake -> advise ->
  ...
  decision to a human operator (office-support operations coordinator).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:flag-confidentiality-concern` ALWAYS reaches this node
  when the governor is clean -- see `ofsup.phase`; `:schedule-service-
  operation`/`:coordinate-supply-order` reach it only when the governor
  and phase gate agree they are not auto-eligible (see `ofsup.governor`
  ns docstring `UNLIKE`)."
...
(defn build
  ...
  `ofsup.store/Store`).
  opts:
    :advisor      -- an `ofsup.ofsupllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (ofsupllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; OfficeSupport-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (ofsupllm/-advise advisor store request)]
            {:proposal p :audit [(ofsupllm/trace request p)]})))

      ;; Office Support Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) :coordination
                                          :else :low-confidence))
                        :phase ph
